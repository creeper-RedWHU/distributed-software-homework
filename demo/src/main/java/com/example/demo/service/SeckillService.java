...
public class SeckillService {
    ...
    public Result<Long> doSeckill(Long userId, Long seckillId) {
        ...
        // 3. Redis Lua脚本原子操作：检查重复下单 + 限购 + 预减库存
        SeckillRedisService.PreDeductResult preDeductResult = seckillRedisService.preDeduct(userId, sp);
        if (preDeductResult == SeckillRedisService.PreDeductResult.SYSTEM_ERROR) {
            return Result.fail(ErrorCode.SYSTEM_ERROR, "系统繁忙，请稍后重试");
        }
        if (preDeductResult == SeckillRedisService.PreDeductResult.DUPLICATE) {
            return Result.fail(ErrorCode.PARAM_ERROR, "不能重复秒杀");
        }
        if (preDeductResult == SeckillRedisService.PreDeductResult.NO_STOCK) {
            return Result.fail(ErrorCode.PARAM_ERROR, "库存不足，秒杀失败");
        }
        if (preDeductResult == SeckillRedisService.PreDeductResult.LIMIT_EXCEEDED) {
            return Result.fail(ErrorCode.PARAM_ERROR, "超过限购数量");
        }
        ...
        if (!seckillOrderProducer.sendSeckillOrder(message)) {
            seckillRedisService.releaseReservation(userId, seckillId, orderId);
            return Result.fail(ErrorCode.SYSTEM_ERROR, "秒杀请求受理失败，请稍后重试");
        }

        // 6. 设置订单状态为处理中
        seckillRedisService.markOrderProcessing(orderId);
    ...
    public Result<Map<String, Object>> getOrderStatus(Long orderId) {
        Object status = seckillRedisService.getOrderStatus(orderId);
        if (status == null) {
            SeckillOrder order = seckillOrderMapper.selectById(orderId);
            if (order != null) {
                seckillRedisService.syncOrderStatus(order);
                SeckillOrderStatus orderStatus = SeckillOrderStatus.fromCode(order.getStatus());
                status = orderStatus != null ? orderStatus.getRedisStatus() : SeckillRedisService.STATUS_UNKNOWN;
            }
        }
        Map<String, Object> data = new HashMap<>();
        data.put("orderId", orderId);
        data.put("status", status != null ? status : SeckillRedisService.STATUS_UNKNOWN);
    ...
    public Result<String> payOrder(Long userId, Long orderId) {
        ...
        if (!order.getUserId().equals(userId)) {
            return Result.fail(ErrorCode.PARAM_ERROR, "用户与订单不匹配");
        }
        if (Objects.equals(order.getStatus(), SeckillOrderStatus.PAID.getCode())) {
            seckillRedisService.markOrderPaid(orderId);
            return Result.success("订单已支付");
        }
        if (!Objects.equals(order.getStatus(), SeckillOrderStatus.PENDING_PAYMENT.getCode())) {
            return Result.fail(ErrorCode.PARAM_ERROR, "订单当前状态不允许支付");
        }

        SeckillOrderPayMessage message = new SeckillOrderPayMessage(orderId, userId, java.time.LocalDateTime.now());
        if (!seckillOrderPayProducer.sendPayOrderMessage(message)) {
            return Result.fail(ErrorCode.SYSTEM_ERROR, "支付受理失败，请稍后重试");
        }
        seckillRedisService.markOrderProcessing(orderId);
        return Result.success("支付请求已受理");
    }
    ...
    public void warmUpSeckillStock(Long seckillId) {
        SeckillProduct sp = seckillProductMapper.selectById(seckillId);
        if (sp != null) {
            seckillRedisService.ensureStockLoaded(sp);
    ...
    private SeckillProductVO buildSeckillVO(SeckillProduct sp, Product product) {
        ...
        vo.setPurchaseLimit(sp.getPurchaseLimit());
